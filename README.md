...
9977번째 세대의 Best는 에러값이 0.650904인 0.370452이나 9988번째 세대의 Best는 에러값이 0.650384인 0.370192로 에러값이 낮아지고 있는 것을 육안으로 볼 수 있다. 이는 세대가 지남에 따라 적합도가 높아지고 있다는 것을 의미한다.  
추가적으로 이때의 마지막 Best값은 0.370192로 어느정도 시간이 지나면 값이 고착되는 것을 알 수 있다.
...
error값이 낮으면 낮을수록 최적화된 값에 부합한하고 할 수 있으며, 만 회와 십만 회 모두 error값이 가장 낮을때의 a값은 0.37이었기에 가장 최적화된 수학적모형은 'y = 0.37 * x + 1'이라 할 수 있다.
